...
function deleteUser(userId) {
  if (
    !confirm(
      "Voulez-vous supprimer cet utilisateur ? Il sera masqué mais conservé dans l'historique."
    )
  )
    return;
  const formData = new FormData();
  formData.append("idUser", userId);
  fetch(getApiUrl("delete_user.php"), { method: "POST", body: formData })
    .then((response) => response.json())
    .then((data) => {
      showNotification(
...
function setupUserDeleteEvents() {
  const container = document.getElementById("usersContainer");
  if (!container || container.dataset.deleteEventsReady === "1") return;

  container.dataset.deleteEventsReady = "1";
