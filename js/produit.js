...
function renderProduit(produit) {
  ...
  const image = produit.image
    ? `<img src="img/${encodeURIComponent(
        String(produit.image)
      )}" alt="${escapeHtml(produit.designation)}">`
    : `<div class="placeholder">📦</div>`;
  card.innerHTML = `
        <div class="produit-image">${image}
         <div class="detail-price">${escapeHtml(
           produit.prixUnitaire
         )} FCFA</div>
        </div>
                    <div class="produit-content">
                        <h3 class="produit-title">${escapeHtml(
                          produit.designation
                        )}</h3>
                        <div>
  ...
  fetch(url, {
    ...
    .then((data) => {
      if (data.success) {
        ...
        showNotification(
          data.message || "Erreur lors de la modification du produit",
          "error"
        );
      }
    })
...
function editProduit(idproduit) {
  ...
  fetch(url)
    ...
    .then((data) => {
      ...
      if (produit.image) {
        previews.innerHTML = `<img src="img/${encodeURIComponent(
          String(produit.image)
        )}" alt="${escapeHtml(produit.designation)}">`;
      }
...
function showImagePreview(file) {
  ...
  reader.onload = function (e) {
    ...
    preview.innerHTML = `<img src="${e.target.result}" alt="Aperçu" style="max-width: 100%; max-height: 200px; border-radius: 8px;">`;
  };
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    .getElementById("updateProduit")
    .addEventListener("click", updateProduit);
